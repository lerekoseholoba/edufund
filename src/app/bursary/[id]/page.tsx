  ...
  if (isLoading) {
    return (
      <main className="mx-auto max-w-3xl px-6 py-10">
        <p className="text-gray-500">Loading bursary details...</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      ...
      <Section title="About this bursary">
        <p className="text-gray-700">{bursary.description}</p>
      </Section>

      <Section title="Funding amount & benefits">
        <ul className="list-disc space-y-1 pl-5 text-gray-700">
          {bursary.benefits.map((benefit, index) => (
      ...
      <Section title="Eligibility">
        <ul className="list-disc space-y-1 pl-5 text-gray-700">
          <li>
            Minimum academic requirement: {bursary.minimumAverage}
          </li>

          {bursary.requirements.map((requirement, index) => (
            <li key={index}>{requirement}</li>
          ))}

          <li>
            Eligible provinces: {bursary.provinces.join(', ')}
          </li>

          <li>
            Household income bracket:{' '}
            {bursary.incomeBracket.join(' or ')}
          </li>
        </ul>
      </Section>

      <Section title="Work-back obligation">
        <p className="text-gray-700">{bursary.workBackObligation}</p>
      </Section>

      <Section title="Documents you'll need to submit">
        <ul className="list-disc space-y-1 pl-5 text-gray-700">
          {bursary.requiredDocuments.map((document, index) => (
